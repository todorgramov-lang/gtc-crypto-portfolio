    ...
    <div className="h-full overflow-y-auto overscroll-contain">
      <header className="sticky top-0 z-10 border-b border-ink-700 bg-ink-900/95 px-4 py-2.5 backdrop-blur">
        <div className="flex items-center justify-between">
          <h1 className="text-sm font-semibold">Транзакции</h1>
          <button type="button" onClick={onAdd} className="px-2 text-lg leading-none">
            +
          </button>
        </div>
        ...
        {groups.length === 0 ? (
          ...
          groups.map(([key, group]) => (
            <section key={key} className="mb-4">
              <h2 className="mb-1 px-1 text-[11px] font-medium uppercase tracking-wide text-fg-faint">
                {group.label}
              </h2>
              <ul>
